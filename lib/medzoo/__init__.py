...
from .Nested_DenseUnet3D import NestedDenseUNet3D
...
model_list = ['UNET3D', 'DENSENET1', "UNET2D", 'DENSENET2', 'DENSENET3', 'HYPERDENSENET', "SKIPDENSENET3D",
              ...
              "HIGHRESNET", "DENSEUNET3D", "NESTEDUNET3D"]
...
def create_model(args):
    ...
    if model_name == 'VNET2':
        ...
        model = ResUNet3D(in_channels=in_channels, n_classes=num_classes, base_n_filter=8)
        ...
        model = DenseUNet3D(in_channels=in_channels, n_classes=num_classes)
    elif model_name == "NESTEDUNET3D":
        model = NestedUNet(in_ch=in_channels, out_ch=num_classes)
    elif model_name == "NESTEDDENSEUNET3D":
        model = NestedDenseUNet3D(in_channels=in_channels, out_channels=num_classes)
