...
class Trainer:
    ...
    def __init__(self, args, model, criterion, optimizer, train_data_loader,
        ...
        self.result_path =f'results/{time.strftime("%Y%m%d-%H%M%S", time.localtime(time.time()))}'
    ...
    def train_epoch(self, epoch):
        ...
        for batch_idx, input_tuple in enumerate(self.train_data_loader):
            ...
            print(f"epoch : {epoch} , batch : {batch_idx}, loss : {loss_dice.item()}, dice : ")
    ...
    def validate_epoch(self, epoch):
        ...
        for batch_idx, input_tuple in enumerate(self.valid_data_loader):
            with torch.no_grad():
                ...
                images = [output.cpu().detach().numpy(),target.cpu().detach().numpy()]

                for i in range(1, columns*rows +1):
                    fig.add_subplot(rows, columns, i)
                    plt.imshow(images[i-1][0,0,:,:,80])
                
                plt.savefig(f'{self.result_path}/{epoch}_{batch_idx}.png')

                loss = self.criterion(output, target)

                print(f"epoch : {epoch} , batch : {batch_idx}, loss : {loss.item()}, dice : ")
