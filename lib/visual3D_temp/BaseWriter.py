...
class TensorboardWriter():
    ...
    def display_terminal(self, iter, epoch, mode='train', summary=False):
        ...
        """
        if summary:
            info_print = "\nSummary {} Epoch {:2d}:  Loss:{:.4f} \t Dise Coefficient:{:.4f}  ".format(mode, epoch,
                                                                                         self.data[mode]['loss'] /
            ...
            info_print = "\nEpoch: {:.2f} Loss:{:.4f} \t Dise Coefficient:{:.4f}".format(iter, self.data[mode]['loss'] /
                                                                            self.data[mode]['count'],
                                                                            self.data[mode]['dsc'] /
                                                                            self.data[mode]['count'])
